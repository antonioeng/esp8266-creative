/**
 * App.jsx — ESP8266 Creative · Root Component
 *
 * Layout principal con diseño cyberpunk neon.
 * Mantiene toda la funcionalidad del simulador original.
 */
...
function getInitialTheme() {
  try {
    return localStorage.getItem("esp-creative-theme") || "dark";
  } catch { return "dark"; }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem("esp-creative-theme", theme); } catch {}
  }, [theme]);
  ...
  useEffect(() => {
    ...
    projectService.loadCurrentProject().then((project) => {
      if (project) {
        ...
        setProjectName(project.name || "My ESP8266 Project");
    ...
    <div className="app">
      ...
      <header className="app-header">
        <div className="header-left">
          <div className="logo">
            ...
            <span className="logo-text">ESP8266 Creative</span>
            <span className="logo-version">v1.0</span>
          </div>
          <button
            className="theme-toggle"
            onClick={toggleTheme}
            title={theme === "dark" ? "Digital Dream ☀️" : "Neon Lab 🌙"}
          >
            {theme === "dark" ? "☀️" : "🌙"}
          </button>
          <a
            className="github-link"
            href="https://github.com/antonioeng/esp8266-creative"
            target="_blank"
        ...
        <div className="header-center">
          <input
            ...
            spellCheck={false}
          />
          {!projectSaved && <span className="unsaved-dot" title="Unsaved changes">●</span>}
          {projectSaved && lastSaved && (
            <span className="saved-indicator" title={`Saved: ${new Date(lastSaved).toLocaleTimeString()}`}>
              ✓
            </span>
          )}
        </div>
        <div className="header-right">
          <span className={`connection-badge ${connectionMode}`}>
            {connectionMode === "simulation" ? "🔬 SIM" : "🔌 HW"}
          </span>
        </div>
      </header>
      ...
      <footer className="app-footer">
        <span className="footer-text">
          DESIGNED BY{" "}
          <a className="footer-link" href="https://github.com/antonioeng" target="_blank" rel="noopener noreferrer">
            ANTONIOENG
          </a>
        </span>
        <span className="footer-text">•</span>
        <span className="footer-text">ESP8266 CREATIVE</span>
      </footer>
    </div>
  );
}
